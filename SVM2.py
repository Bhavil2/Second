...
scaled_data = scaler.fit_transform(ph_2_data1)

# Train One-Class SVM
svm = OneClassSVM(nu=0.05, kernel='rbf')  # nu=0.05 means we expect 5% outliers
svm.fit(scaled_data)
...
# Create DataFrame with outlier flags for each procedure in each cluster
for i in range(84):
    cluster_outliers = outlier_mask[i]
    ph_2_data2.append(cluster_outliers)
...
for i in range(84):
    for j in range(10):
        if ph_2_data2.iloc[i,j] == True:
            ph_3_data1.loc[len(ph_3_data1)] = [i, ph_2_data2.columns[j]]
...
print("Code executed successfully using SVM") 
